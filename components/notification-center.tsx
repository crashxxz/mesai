...
export function NotificationCenter() {
  ...
  return <>
    <div className="relative">
      ...
      {open ? <div className="absolute right-0 top-12 z-50 w-[min(92vw,360px)] rounded-2xl border border-slate-200 bg-white p-3 shadow-soft-lg">
        ...
        <div className="mt-2 max-h-80 overflow-auto">{notices.length ? notices.map((notice) => <button key={notice.id} className={`block w-full rounded-xl p-3 text-left text-sm ${read.includes(notice.id) ? "text-slate-500" : "bg-amber-50 font-bold text-slate-900"}`} onClick={() => { const next = [...new Set([...read, notice.id])]; setRead(next); localStorage.setItem(storageKey, JSON.stringify(next)); }}><span>{notice.text}</span><small className="mt-1 block text-xs text-slate-400">{relativeTime(notice.at)}</small></button>) : <p className="p-5 text-center text-sm font-bold text-slate-400">Tudo tranquilo por aqui.</p>}</div>
      </div> : null}
    </div>
    {toast ? <div className="fixed right-4 top-16 z-[60] max-w-xs rounded-xl bg-slate-950 px-4 py-3 text-sm font-bold text-white shadow-soft-lg">{toast}</div> : null}
  </>;
}
