...
export function NotificationCenter() {
  ...
  const storageKey = `mesai-notices-${profile?.id ?? "guest"}-${restaurant?.id ?? "none"}`;
  const clearKey = `${storageKey}-cleared`;
  const allNotices = useMemo(
    () => buildNotices(state, restaurant?.id).filter((notice) => notice.roles.some((role) => roles.includes(role))).slice(0, 12),
    [restaurant?.id, roles, state]
  );
  ...
  function persist(nextRead: string[], nextCleared = cleared) {
    setRead(nextRead);
    setCleared(nextCleared);
    localStorage.setItem(storageKey, JSON.stringify(nextRead));
    localStorage.setItem(clearKey, JSON.stringify(nextCleared));
  }

  function markAllRead() {
    persist(notices.map((notice) => notice.id));
  }

  function clearVisibleNotifications() {
    const ids = notices.map((notice) => notice.id);
    persist([...new Set([...read, ...ids])], [...new Set([...cleared, ...ids])]);
  }

  function clearNotification(id: string) {
    persist([...new Set([...read, id])], [...new Set([...cleared, id])]);
  }
      ...
      {open ? <div className="fixed inset-x-3 top-16 z-50 flex max-h-[calc(100dvh-5rem)] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-3 shadow-soft-lg sm:absolute sm:inset-x-auto sm:right-0 sm:top-12 sm:w-[360px]">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1">
            <button title="Som" className="grid h-9 w-9 place-items-center rounded-lg hover:bg-slate-100" onClick={() => { const next = !sound; setSound(next); localStorage.setItem(`${storageKey}-sound`, next ? "1" : "0"); }}>{sound ? <Volume2 className="h-4 w-4" /> : <VolumeX className="h-4 w-4" />}</button>
            <button title="Marcar todas como lidas" className="grid h-9 w-9 place-items-center rounded-lg hover:bg-slate-100" onClick={markAllRead}><CheckCheck className="h-4 w-4" /></button>
            <button title="Limpar todas" className="grid h-9 w-9 place-items-center rounded-lg hover:bg-slate-100" onClick={clearVisibleNotifications}><Trash2 className="h-4 w-4" /></button>
          </div>
        </div>
        ...
        <div className="mt-2 min-h-0 flex-1 overflow-y-auto overscroll-contain">
          {notices.length ? notices.map((notice) => {
            const nextRead = [...new Set([...read, notice.id])];
            return (
              <div key={notice.id} className={`mb-2 flex items-start gap-2 rounded-xl p-3 text-sm ${read.includes(notice.id) ? "text-slate-500" : "bg-amber-50 font-bold text-slate-900"}`}>
                <button type="button" className="min-w-0 flex-1 text-left" onClick={() => persist(nextRead)}>
                  <span>{notice.text}</span>
                  <small className="mt-1 block text-xs text-slate-400">{relativeTime(notice.at)}</small>
                </button>
                <button type="button" title="Remover notificação" aria-label="Remover notificação" className="-mr-1 -mt-1 grid h-8 w-8 shrink-0 place-items-center rounded-lg hover:bg-white/70" onClick={() => clearNotification(notice.id)}>
                  <X className="h-4 w-4" />
                </button>
              </div>
            );
          }) : <p className="p-5 text-center text-sm font-bold text-slate-400">Tudo tranquilo por aqui.</p>}
        </div>
      </div> : null}
    </div>
