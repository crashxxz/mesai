...
function buildNotices(state: ReturnType<typeof useStore>["state"], restaurantId?: string): Notice[] {
  ...
  for (const item of state.orderItems.filter((entry) => entry.restaurantId === restaurantId && entry.status === "cancelled")) notices.push({ id: `rejected-${item.id}`, text: `Item recusado: ${item.productNameSnapshot}. Motivo: ${item.cancelReason ?? "sem motivo"}`, at: item.updatedAt, roles: ["owner", "manager", "waiter"] });
