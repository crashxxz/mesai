...
function mapPublicMenu(base: AppState, payload: Record<string, unknown>): AppState {
  ...
  const products = ((payload.products ?? []) as Record<string, unknown>[]).map((p): Product => {
    ...
    return { id: String(p.id), restaurantId, categoryId: String(p.category_id), name: String(p.name), description: p.description ? String(p.description) : undefined, price: Number(p.price), preparationSector: p.preparation_sector as Product["preparationSector"], estimatedTimeMinutes: p.estimated_time_minutes ? Number(p.estimated_time_minutes) : undefined, available: p.available !== false, hasStockControl: false, imageUrl: manualImageUrl, generatedImageUrl: typeof p.generated_image_url === "string" ? p.generated_image_url : manualImageUrl ? undefined : resolveProductImage(p, categoryName) ?? undefined, active: p.active !== false, createdAt: String(p.created_at ?? now), updatedAt: String(p.updated_at ?? now) };
  });
