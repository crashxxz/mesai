-- Fix: register_order_payment MUST require an open cash session.
-- If no cash session is open, payment is rejected with clear error.
-- This ensures all payments are tracked in the cash register.
...
begin
  ...
  if v_order.id is null or v_order.restaurant_id <> v_profile.restaurant_id or v_order.status in ('closed', 'cancelled') then
  ...
  if v_profile.role not in ('owner', 'manager', 'cashier') and not (v_profile.role = 'waiter' and v_waiter_allowed) then
  ...
  if v_cash_session.id is null then
    raise exception 'Caixa fechado. Abra o caixa antes de registrar pagamentos.';
  end if;
  ...
  values (v_profile.restaurant_id, v_profile.id, 'payment_registered', 'payments', v_payment_id,
    jsonb_build_object('order_id', p_order_id, 'method', p_method, 'amount', v_amount, 'cash_session_id', v_cash_session.id));

  return v_payment_id;
end;
$$;

-- Also ensure open_cash_session uses coalesce for waiter_can_close check
-- and has proper error messages (already defined in APPLY_MESAI_PROD, just re-grant)
