...
-- ===== 9. MIGRATION: 202606210002_critical_flows.sql =====
create or replace function public.get_public_restaurant_tables(p_slug text)
...
begin
  ...
  if v_restaurant.id is null then raise exception 'Estabelecimento nÃ£o encontrado'; end if;
  ...
  if v_table.id is null then raise exception 'Mesa nÃ£o encontrada'; end if;
  select qr_orders_enabled into v_enabled from public.restaurant_settings where restaurant_id = v_restaurant.id;
  if not coalesce(v_enabled, false) then raise exception 'Pedido por QR indisponÃ­vel'; end if;
...
begin
  select * into v_profile from public.profiles where user_id = auth.uid() and active and deleted_at is null limit 1;
  if v_profile.id is null or not (public.current_roles() && array['owner','manager','waiter']::public.profile_role[]) then raise exception 'Sem permissÃ£o'; end if;
...
begin
  ...
  if v_order.id is null then raise exception 'Pedido nÃ£o encontrado'; end if;
  ...
  if v_product.id is null then raise exception 'Produto indisponÃ­vel'; end if;
...
declare v_profile public.profiles%rowtype; v_order public.orders%rowtype; v_percent numeric(5,2); v_fee numeric(12,2) := 0;
begin
  select * into v_profile from public.profiles where user_id = auth.uid() and active limit 1;
  ...
  if p_enabled then select service_fee_percent into v_percent from public.restaurant_settings where restaurant_id = v_profile.restaurant_id; v_fee := round(greatest(v_order.subtotal - v_order.discount,0) * (coalesce(v_percent,0)/100),2); end if;
  update public.orders set service_fee_enabled = p_enabled, service_fee = v_fee, total = greatest(subtotal-discount,0) + v_fee + coalesce(delivery_fee,0), updated_at = now() where id = p_order_id;
  return p_order_id;
end;
$$;
...
-- Aplicar por último: restaura a mesa como ocupada também quando já existe comanda aberta.
create or replace function public.ensure_open_table_order(p_table_id uuid)
returns uuid language plpgsql security definer set search_path = public, extensions as $$
declare v_profile public.profiles%rowtype; v_order_id uuid; v_tab_id uuid;
begin
  ...
  select id into v_order_id from public.orders where table_id = p_table_id and restaurant_id = v_profile.restaurant_id and status not in ('closed','cancelled') order by created_at desc limit 1;
  if v_order_id is not null then update public.tables set status = 'occupied', updated_at = now() where id = p_table_id; return v_order_id; end if;
  update public.qr_sessions set active = false where table_id = p_table_id and active;
  select id into v_tab_id from public.tabs where table_id = p_table_id and restaurant_id = v_profile.restaurant_id and status = 'open' order by opened_at desc limit 1;
  if v_tab_id is null then insert into public.tabs (restaurant_id, table_id, opened_by) values (v_profile.restaurant_id, p_table_id, v_profile.id) returning id into v_tab_id; end if;
  insert into public.orders (restaurant_id, table_id, tab_id, source, status, created_by) values (v_profile.restaurant_id, p_table_id, v_tab_id, 'waiter', 'open', v_profile.id) returning id into v_order_id;
...
declare v_token public.table_qr_tokens%rowtype; v_table public.tables%rowtype; v_session text := replace(gen_random_uuid()::text, '-', '');
begin
  select * into v_token from public.table_qr_tokens where token_hash = digest(p_table_token, 'sha256') and active and (expires_at is null or expires_at > now());
  ...
  insert into public.qr_sessions (restaurant_id, table_id, session_hash, active, expires_at) values (v_token.restaurant_id, v_token.table_id, digest(v_session, 'sha256'), true, now() + interval '12 hours');
  return v_session;
end;
$$;
grant execute on function public.ensure_open_table_order(uuid) to authenticated;
...
-- Reparo final: uma comanda existente também restaura a mesa para ocupada.
create or replace function public.ensure_open_table_order(p_table_id uuid)
returns uuid language plpgsql security definer set search_path = public, extensions as $$
declare v_profile public.profiles%rowtype; v_order_id uuid; v_tab_id uuid;
begin
...
begin
  select * into v_profile from public.profiles where user_id = auth.uid() and active and deleted_at is null limit 1;
  if v_profile.id is null or not (public.current_roles() && array['owner','manager']::public.profile_role[]) then raise exception 'Sem permissÃ£o'; end if;
  if exists(select 1 from public.order_items where product_id = p_product_id) then
    ...
    if not found then raise exception 'Produto nÃ£o encontrado'; end if;
    return 'inactivated';
  end if;
...
-- ===== 11. CORREÇÕES CRÍTICAS DE QR, FECHAMENTO E ESTOQUE =====
alter function public.rotate_table_qr_token(uuid) set search_path = public, extensions;
...
begin
  ...
  if v_paid < v_order.total then raise exception 'Pagamento incompleto'; end if;
  update public.orders set status = 'closed', closed_by = v_profile.id, closed_at = now() where id = p_order_id;
  if v_order.tab_id is not null and not exists (select 1 from public.orders where tab_id = v_order.tab_id and status not in ('closed', 'cancelled')) then update public.tabs set status = 'closed', closed_by = v_profile.id, closed_at = now() where id = v_order.tab_id; end if;
...
returns uuid language plpgsql security definer set search_path = public, extensions as $$
declare v_profile public.profiles%rowtype; v_order public.orders%rowtype; v_percent numeric(5,2); v_fee numeric(12,2);
begin
  select * into v_profile from public.profiles where user_id = auth.uid() and active limit 1;
  if v_profile.id is null or not (public.current_roles() && array['owner','manager','waiter']::public.profile_role[]) then raise exception 'Sem permissao'; end if;
  select * into v_order from public.orders where id = p_order_id and restaurant_id = v_profile.restaurant_id and status not in ('closed', 'cancelled') for update;
  ...
  update public.orders set service_fee_enabled = true, service_fee = v_fee, total = greatest(v_order.subtotal - v_order.discount, 0) + v_fee + coalesce(v_order.delivery_fee, 0), updated_at = now() where id = p_order_id;
  return p_order_id;
end;
$$;
...
-- A taxa configurada vale por padrão e pode ser removida por comanda.
create or replace function public.add_order_item(p_order_id uuid, p_product_id uuid, p_quantity numeric, p_notes text)
returns uuid language plpgsql security definer set search_path = public, extensions as $$
declare v_profile public.profiles%rowtype; v_order public.orders%rowtype; v_product public.products%rowtype; v_item_id uuid; v_subtotal numeric; v_service numeric := 0;
begin
  select * into v_profile from public.profiles where user_id = auth.uid() and active and deleted_at is null limit 1;
  if v_profile.id is null or not (public.current_roles() && array['owner','manager','waiter']::public.profile_role[]) then raise exception 'Sem permissao'; end if;
  select * into v_order from public.orders where id = p_order_id and restaurant_id = v_profile.restaurant_id and status not in ('closed','cancelled') for update;
  ...
  values (v_order.id, v_profile.restaurant_id, v_product.id, v_product.name, v_product.price, greatest(p_quantity,1), nullif(trim(p_notes),''), v_product.preparation_sector, 'pending', v_profile.id) returning id into v_item_id;
  select coalesce(sum((unit_price_snapshot + coalesce(variation_price_delta,0)) * quantity),0) into v_subtotal from public.order_items where order_id = v_order.id and status <> 'cancelled';
  if coalesce(v_order.service_fee_enabled, true) then select round(v_subtotal * (service_fee_percent / 100), 2) into v_service from public.restaurant_settings where restaurant_id = v_profile.restaurant_id; end if;
