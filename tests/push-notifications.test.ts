...
test("eventos reais de push cobrem QR, chamados, preparo, pronto e recusa", () => {
  ...
  assert.match(eventRoute, /rolesForPreparationSector/);
  assert.match(publicEventRoute, /Novo pedido pelo QR/);
  assert.match(publicEventRoute, /Cliente chamou atendimento/);
  assert.match(publicEventRoute, /Cliente pediu conta/);
});

test("ativação push fica no sino e não depende de Ajustes", () => {
